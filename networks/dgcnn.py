...
from torch_geometric.nn import radius_graph, knn_graph, EdgeConv
...
class DGCNNCls(BaseNet):
    def __init__(self, num_classes, R=0.05, conv_aggr = 'max', device=None):
        super().__init__()
        print('Build up DGCNNCls model, num_classes {}, R {}'.format(num_classes, R))
        self.R = R
    ...
    def forward(self, pts, batch_ids):
        ...
        out = pts
        edge_conv_outs = []
        #new line for not calcuting the new graph after each iteration
        #edge_index = radius_graph(pts, self.R, batch=batch_ids, max_num_neighbors=1024)
        for edge_conv in self.edge_convs.children():
            # Dynamically update graph, pretty sure that this line is BS since it uses pts to compute the graph
            edge_index = radius_graph(pts, self.R, batch=batch_ids, max_num_neighbors=1024)
        ...
        #out = torch.scatter('max', out, index=batch_ids, dim=1)  # B, 1024 
        #out = scatter_('max', out, index=batch_ids, dim_size=batch_size)  # B, 1024, original line
        
        out,hi = scatter_max(src=out, index=batch_ids, dim_size=batch_size, dim=0)
        #HIER CHANGED
...
class DGCNNSeg(BaseNet):
    def __init__(self, num_classes, R=0.05, conv_aggr='max', device=None):
        super().__init__()
        print('Build up DGCNNSeg model, num_classes {}, R {}'.format(num_classes, R))
        self.R = R
        self.num_classes = num_classes
        self.conv_aggr = conv_aggr

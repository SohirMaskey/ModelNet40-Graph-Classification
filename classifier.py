...
def main(config):    
    ...
    out_dir = os.path.join(config.odir, config.network, config.dataset, 'R{}_{}'.format(config.R, optim_tag))
    ...
    if config.training:
        ...
        # Initialize network
        net = DGCNNCls(num_classes=num_classes, R=config.R, device=device)
        ...
        for epoch in range(last_epoch+1, max_epoch+1):
            ...
            if config.val and epoch % config.val == 0:
                ...
                train_acc = test_epoch(net, train_loader, get_label_fn)
                ...
                lprint('Save {} val acc:{:.4f}, train acc:{:.4f}'.format(ckpt_name, val_acc, train_acc), log)
            #visdom.save_state()
        ...
        testacc = test_epoch(net, test_loader, get_label_fn)
        lprint('Accuracy: {:.4f}, Acc2: {:.4f}, time {:.4f}s\n\n'.format(test_acc, testacc, time.time() - start_time),  log)  
    log.close()
